    ...
    <DropdownMenu ref="dropdown" slot="list">
      <template v-for="child in children">
        ...
        <DropdownItem v-else :key="`drop-${child.name}`" :name="child.name">
          <custom-and-default-icon :size="iconSize" :type="child.icon" />
...
import { NodeHelperUtil } from '@/common/util/NodeHelperUtil'
...
export default class CollapsedMenu extends mixins(Mixin, ItemMixin) {
  ...
  mounted () {
    let dropdown = NodeHelperUtil.findNodeUpperByClasses((this.$refs.dropdown as Vue).$el, [
      'ivu-select-dropdown',
