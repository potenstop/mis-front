...
try {
  (function (window) {
    ...
    function HSTS_Cookie (domains) {
      ...
      return {
        ...
        'set_hsts_value': function (values, callback) {
          ...
          for (var i = 0; i < domains.length; ++i) {
            fields.push(undefined)
            if (values[i]) { create_request(i, domains[i] + '?SET=1', callback) } else { create_request(i, domains[i] + '?DEL=1', callback) }
          }
          return true
        },
    ...
    function Evercookie (options) {
      ...
      var _ec_swf_file_name = opts.swfFileName
      var _ec_xap_file_name = opts.xapFileName
      var _ec_jnlp_file_name = opts.jnlpFileName
      var _ec_hsts = opts.hsts
      ...
      this._utf8_encode = function (str) {
        str = str.replace(/\r\n/g, '\n')
        var utftext = ''; var i = 0; var n = str.length; var c
        for (; i < n; i++) {
          c = str.charCodeAt(i)
          if (c < 128) {
